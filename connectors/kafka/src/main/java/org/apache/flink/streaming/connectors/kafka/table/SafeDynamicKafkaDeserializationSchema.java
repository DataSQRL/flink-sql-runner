...
class SafeDynamicKafkaDeserializationSchema implements KafkaDeserializationSchema<RowData> {
  ...
  private static final class OutputProjectionCollector implements Collector<RowData>, Serializable {
    ...
    private void emitRow(
        @Nullable GenericRowData physicalKeyRow, @Nullable GenericRowData physicalValueRow) {
      ...
      for (int metadataPos = 0; metadataPos < metadataArity; metadataPos++) {
        producedRow.setField(
            physicalArity + metadataPos, metadataConverters[metadataPos].read(inputRecord));
      }
